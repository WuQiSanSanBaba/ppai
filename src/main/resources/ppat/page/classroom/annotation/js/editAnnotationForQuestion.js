async function saveannotationsEditor() {
    ...
    for (let item of editorContainerArray) {
        ...
        const newArray1 = coreJsonArray.filter((item, index) => {
            return coreJsonArray.indexOf(item) === index
        })
        annotation.coreJsonArray = JSON.stringify(newArray1)

        const newArray2 = geneJsonArray.filter((item, index) => {
            return geneJsonArray.indexOf(item) === index
        })
        annotation.geneJsonArray = JSON.stringify(newArray2)
